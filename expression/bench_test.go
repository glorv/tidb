...
func (s *testVectorizeSuite4) TestVecEvalBool(c *C) {
	ctx := mock.NewContext()
...
func (s *testVectorizeSuite4) TestVecToBool(c *C) {
	ctx := mock.NewContext()
...
func (s *testVectorizeSuite4) TestRowBasedFilterAndVectorizedFilter(c *C) {
	ctx := mock.NewContext()
	eTypes := []types.EvalType{types.ETInt, types.ETReal, types.ETDecimal, types.ETString, types.ETTimestamp, types.ETDatetime, types.ETDuration}
	for numCols := 1; numCols <= 5; numCols++ {
		for round := 0; round < 16; round++ {
...
func (s *testVectorizeSuite4) TestVectorizedFilterConsiderNull(c *C) {
	ctx := mock.NewContext()
